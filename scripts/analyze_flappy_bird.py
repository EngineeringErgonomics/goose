...
def find_flappy_bird_results(benchmark_dir: str) -> List[str]:
    """Find all eval_result.json files for flappy_bird evaluation."""
    pattern = os.path.join(benchmark_dir, "**", "vibes", "flappy_bird", "eval_result.json")
...
def analyze_flappy_bird(base_dir: str, output_csv: str) -> None:
    """Analyze all flappy_bird results and write to CSV."""
    ...
    for benchmark_dir in benchmark_dirs:
        ...
        flappy_results = find_flappy_bird_results(benchmark_dir)
        if flappy_results:
            print(f"Found {len(flappy_results)} flappy_bird results for {provider}-{model}")
            
            for result_file in flappy_results:
    ...
    if csv_rows:
        ...
        print(f"Flappy Bird analysis complete. Results saved to {output_csv}")
    else:
        print("No flappy_bird evaluation results found")

def main():
    import argparse
    
    parser = argparse.ArgumentParser(description='Analyze flappy_bird benchmark results')
    parser.add_argument('--base-dir', default='.', 
                        help='Base directory containing benchmark-provider-model directories')
    parser.add_argument('--output', default='flappy-bird-analysis.csv',
                        help='Output CSV file for analysis results')
    
    args = parser.parse_args()
    analyze_flappy_bird(args.base_dir, args.output)
